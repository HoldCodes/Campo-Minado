    ...
    def __init__(self):
        ...
        for i in range(0, self.tab_x):
            for j in range(0, self.tab_y):
                self.btn = Button(self.container2, width=4, height=2, command=lambda x = i, y = j: self.analisa(x, y), bg="gainsboro", font="BOLD")
    ...
    def jogo(self):
        ...
        for i in range(self.tab_x):
            for j in range(self.tab_y):
                ...
                #if not self.tabuleiro[i][j].bomba:
                    #self.tabuleiro[i][j].botao.configure(text=f"{self.tabuleiro[i][j].bombas_em_volta}")
